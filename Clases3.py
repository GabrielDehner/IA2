class Cluster:
    def __init__(self, id_cluster, color, dimension, tipo_distancia):
...
####
    ...
    def distancia_euclidiana(self, coordenadas_cluster_x, coordenadas_cluster_propio):
        resultado = 0
        if self.tipo_distancia=='E':
            for i in range(len(coordenadas_cluster_x)):
                resultado += (coordenadas_cluster_x[i]- coordenadas_cluster_propio[i])**2
            resultado = resultado**(1/2)
        else:
            if self.tipo_distancia=='M':
                for i in range(len(coordenadas_cluster_x)):
                    resultado += abs(coordenadas_cluster_x[i] - coordenadas_cluster_propio[i])
            else:
                if self.tipo_distancia=="C":
                    for i in range(len(coordenadas_cluster_x)):
                        if(resultado < (coordenadas_cluster_x[i] - coordenadas_cluster_propio[i])):
                            resultado = (coordenadas_cluster_x[i] - coordenadas_cluster_propio[i])
        return resultado
...
class Lista_Cluster:
    ...
    def imprimir_lista(self):
        ...
        print("----------------------")
        for i in range(self.tamanio_contenedor()):
            print(self.contenedor[i].id_cluster)
            for h in range(len(self.contenedor[i].lista_puntos)):
                #hacer que ande en 3d dinamicamente
                print("\t",  self.contenedor[i].lista_puntos[h][1], " " , self.contenedor[i].lista_puntos[h][2], " ", self.contenedor[i].cluster_dependiente)
            print("*********")